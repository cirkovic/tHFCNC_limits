...
if [[ "$HOSTNAME" == "cmsphys05" ]]; then
    ...
    T="/afs/cern.ch/work/c/cirkovic/fcnc_ana/CMSSW_8_0_12/src/tHFCNC/NtupleAnalyzer/test/NAOUTPUT"
    J=4;
fi
...
if [[ "$SCENARIO" != "" ]]; then
    ...
    OPTIONS=" -P $T -j $J -l 12.9 -f  --od cards/${DATE} --asimov ";
    #OPTIONS=" -P $T -j $J -l 12.9 -f  --od cards/16-11-2016 ";
fi
...
rm cards/${DATE}/*.root cards/${DATE}/*.txt
...
for S1 in Hut Hct; do
    for S2 in ST TT; do
        ...
        for c in b2j3 b2j4 b3j3 b3j4; do
            COMMAND="python makeShapeCards_modified_k.py mca_${S1}${S2}_${DATE}.txt cuts.txt 'mva${S1}${S2}' '10,-1.0,1.0' ${OPTIONS} --tree tr_all_${c} $FTREES ${SYSTS/.txt/_}${S1}${S2}_${DATE}.txt -o mva${S1}${S2}all${c} --s2v"
            echo $COMMAND
            eval $COMMAND &
            PSs="$PSs $!"
