...
if [[ "$SCENARIO" != "" ]]; then
    ...
    OPTIONS=" -P $T -j $J -l 12.9 -f  --od cards/16-11-2016 --asimov ";
...
rm cards/16-11-2016/*.root cards/16-11-2016/*.txt
...
for S1 in Hut Hct; do
    for S2 in ST TT; do
        ...
        for c in b2j3 b2j4 b3j3 b3j4; do
            COMMAND="python makeShapeCards_modified_k.py mca_${S1}${S2}_16-11-2016.txt cuts.txt 'mva${S1}${S2}' '10,-1.0,1.0' ${OPTIONS} --tree tr_all_${c} $FTREES ${SYSTS/.txt/_}${S1}${S2}.txt -o mva${S1}${S2}all${c} --s2v"
            echo $COMMAND
            eval $COMMAND &
            PSs="$PSs $!"
