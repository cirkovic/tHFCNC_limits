...
from CMGTools.TTHAnalysis.tools.MVAVars_b3j4 import MVAVars_b3j4
MODULES.append( ('MVAVars_b3j4', lambda : MVAVars_b3j4()) )
...
for D in glob(args[0]+"/*"):
    ...
    if os.path.exists(fname) or (os.path.exists("%s/%s/tree.root.url" % (D,options.tree))):
        ...
        data =  any(x in short for x in "SingleMuonB SingleElectronB SingleMuonC SingleElectronC SingleMuonD SingleElectronD".split()) # FIXME
