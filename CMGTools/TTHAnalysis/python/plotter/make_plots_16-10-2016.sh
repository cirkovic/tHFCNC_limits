...
for SIG in ST TT; do
    ...
    for c in b2j4 b3j3 b3j4; do
        COMMAND="python mcPlots_${DATE//-/}.py -P ${INPUT} -j 8 -l 12.9 -f --maxRatioRange 0 3.7 --poisson ${MCA} cuts.txt plots_${DATE}.txt --pdir ${OUTPUT}/${SIG}/${c} --tree tr_all_${c} $OPTIONS"
        echo $COMMAND
        eval $COMMAND &
    done
