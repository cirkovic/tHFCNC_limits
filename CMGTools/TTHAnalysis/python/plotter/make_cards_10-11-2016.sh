...
rm cards/10-11-2016/*.root cards/10-11-2016/*.txt
...
for SIG in ST TT; do
    ...
    for c in b2j4 b3j3 b3j4; do
        ...
        #COMMAND="python makeShapeCards_modified_k.py mca_${SIG}_10-11-2016.txt cuts.txt 'MVA_TOPHLEPBB' '4,-0.6,0.6' ${OPTIONS} --tree tr_all_${c} $FTREES ${SYSTS/.txt/_}${SIG}.txt -o MVA_TOPHLEPBB_${SIG}_${c} --s2v"
        #COMMAND="python makeShapeCards_modified_k.py mca_${SIG}_10-11-2016.txt cuts.txt 'MVA_TOPHLEPBB' '6,-0.8,0.8' ${OPTIONS} --tree tr_all_${c} $FTREES ${SYSTS/.txt/_}${SIG}.txt -o MVA_TOPHLEPBB_${SIG}_${c} --s2v"
        COMMAND="python makeShapeCards_modified_k.py mca_${SIG}_10-11-2016.txt cuts.txt 'MVA_TOPHLEPBB' '20,-1.0,1.0' ${OPTIONS} --tree tr_all_${c} $FTREES ${SYSTS/.txt/_}${SIG}.txt -o MVA_TOPHLEPBB_${SIG}_${c} --s2v"
        echo $COMMAND
        eval $COMMAND &
        PSs="$PSs $!"
