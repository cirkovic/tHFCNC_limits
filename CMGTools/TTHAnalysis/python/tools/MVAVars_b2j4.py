...
class MVAVars_b2j4:
    def __init__(self):
        ...
        self._all_vars = [
            ...
            MVAVar("chi2_TOPTOPLEPHBB:=chi2_TOPTOPLEPHBB" , func = lambda ev : ev.chi2_TOPTOPLEPHBB),
            MVAVar("chi2_TOPHLEPBB:=chi2_TOPHLEPBB" , func = lambda ev : ev.chi2_TOPTOPLEPHAD),
            MVAVar("chi2_TOPTOPLEPHAD:=chi2_TOPTOPLEPHAD" , func = lambda ev : ev.chi2_TOPTOPLEPHAD),
            MVAVar("TopLepPt_TOPTOPLEPHAD:=TopLepPt_TOPTOPLEPHAD" , func = lambda ev : ev.TopLepPt_TOPTOPLEPHAD),
            MVAVar("TopLepEta_TOPTOPLEPHAD:=TopLepEta_TOPTOPLEPHAD" , func = lambda ev : ev.TopLepEta_TOPTOPLEPHAD),
        ]
        ...
        trainings = [ "HutST", "HutTT", "HctST", "HctTT" ]
        #samples = ["ST", "TT"]
        categories = ["b2j4"]
        ...
        for s in trainings:
            for c in categories:
                ...
                weights = basedir+"/"+s+"_"+c+"/weights/TMVA_"+s+"_"+c+"_BDT.weights.xml"
                print weights
        ...
        for s in trainings:
            for c in categories:
                ...
                ( cat_lambdas[c], MVATool("MVA_"+s+"_"+c, basedir+"/"+s+"_"+c+"/weights/TMVA_"+s+"_"+c+"_BDT.weights.xml", self._vars[s+"_"+c]) ),
                ])
    ...
    def __call__(self,event):
        ret = dict([ (name, mva(event)) for name, mva in self._MVAs.iteritems()])
        print ret
        return ret
